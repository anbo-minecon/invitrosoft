        ...
        while ($row = $res->fetch_assoc()) {
            ...
            if ($row['tipo'] !== 'soluciones-desinfectantes') {
                ...
                while ($g = $resg->fetch_assoc()) {
                    $reactivos = [];
                    // Sólo traer reactivos activos
                    $stmtr = $conn->prepare("SELECT gr.*, r.nombre_comun FROM grupo_reactivos gr JOIN reactivos r ON gr.reactivo_id = r.id WHERE gr.grupo_id=? AND r.estado = 'activo'");
                    ...
                    while ($r = $resr->fetch_assoc()) {
                        $reactivos[] = [
                            ...
                            "cantidad" => $r['cantidad'],
                            "solucion_madre" => $r['solucion_madre'] ?? null
                        ];
                    }
        ...
        if ($stmt->execute()) {
            ...
            if ($tipo !== 'soluciones-desinfectantes') {
                foreach ($grupos as $g) {
                    ...
                    $volumen_g = $g['volumen'] ?? null;
                    $stmtg = $conn->prepare("INSERT INTO grupos_formulacion (formulacion_id, nombre_grupo, volumen) VALUES (?, ?, ?)");
                    $stmtg->bind_param("iss", $formulacion_id, $nombre_grupo, $volumen_g);
                    $stmtg->execute();
                    $grupo_id = $conn->insert_id;
                    foreach ($g['reactivos'] as $r) {
                        $reactivo_id = intval($r['reactivo_id']);
                        $cantidad = $r['cantidad'] ?? '';
                        $solucion_madre_r = $r['solucion_madre'] ?? null;
                        $stmtr = $conn->prepare("INSERT INTO grupo_reactivos (grupo_id, reactivo_id, cantidad, solucion_madre) VALUES (?, ?, ?, ?)");
                        $stmtr->bind_param("iiss", $grupo_id, $reactivo_id, $cantidad, $solucion_madre_r);
                        $stmtr->execute();
                    }
                }
            }
        ...
        if ($stmt->execute()) {
            ...
            if ($tipo !== 'soluciones-desinfectantes') {
                ...
                foreach ($grupos as $g) {
                    ...
                    $volumen_g = $g['volumen'] ?? null;
                    $stmtg = $conn->prepare("INSERT INTO grupos_formulacion (formulacion_id, nombre_grupo, volumen) VALUES (?, ?, ?)");
                    $stmtg->bind_param("iss", $id, $nombre_grupo, $volumen_g);
                    $stmtg->execute();
                    $grupo_id = $conn->insert_id;
                    foreach ($g['reactivos'] as $r) {
                        $reactivo_id = intval($r['reactivo_id']);
